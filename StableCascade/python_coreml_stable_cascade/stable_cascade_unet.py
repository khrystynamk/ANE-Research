...
class StableCascadeUNet(ModelMixin, ConfigMixin, FromOriginalUNetMixin):
    ...
    def __init__(
        ...
        # CHANGED
        self.clip_txt_pooled_mapper = nn.Conv2d(clip_text_pooled_in_channels, conditioning_dim * clip_seq, kernel_size)
    ...
    def get_clip_embeddings(self, clip_txt_pooled, clip_txt=None, clip_img=None):
        ...
        # DEBUG
        # self.clip_txt_pooled_mapper.kernel_size = 1
        # print(f"AAAAAAAAAAAAA\n\n\n{self.clip_txt_pooled_mapper}\n\n")
...
#             # nn.Linear(c + c_skip, c * 4),
#             nn.Conv2d(
#                 in_channels=c + c_skip, out_channels=c * 4, kernel_size=kernel_size
#             ),
#             nn.GELU(),
#             GlobalResponseNorm(c * 4),
#             nn.Dropout(dropout),
#             # nn.Linear(c * 4, c),
#             nn.Conv2d(in_channels=c * 4, out_channels=c, kernel_size=kernel_size),
#         )
...
#         self.clip_txt_pooled_mapper = nn.Conv2d(
#             in_channels=clip_text_pooled_in_channels,
#             out_channels=conditioning_dim * clip_seq,
#             kernel_size=kernel_size,
#         )
#         if clip_text_in_channels is not None:
#             # self.clip_txt_mapper = nn.Linear(clip_text_in_channels, conditioning_dim)
#             self.clip_txt_mapper = nn.Conv2d(
#                 in_channels=clip_text_in_channels,
#                 out_channels=conditioning_dim,
#                 kernel_size=kernel_size,
#             )
#         if clip_image_in_channels is not None:
#             # self.clip_img_mapper = nn.Linear(clip_image_in_channels, conditioning_dim * clip_seq)
#             self.clip_img_mapper = nn.Conv2d(
#                 in_channels=clip_image_in_channels,
#                 out_channels=conditioning_dim * clip_seq,
#                 kernel_size=kernel_size,
#             )
...
#         nn.init.normal_(self.clip_txt_pooled_mapper.weight, std=0.02)
#         (
#             nn.init.normal_(self.clip_txt_mapper.weight, std=0.02)
#             if hasattr(self, "clip_txt_mapper")
#             else None
#         )
#         (
#             nn.init.normal_(self.clip_img_mapper.weight, std=0.02)
...
#             for block in level_block:
#                 if isinstance(block, SDCascadeResBlock):
#                     block.channelwise[-1].weight.data *= np.sqrt(
#                         1 / sum(self.config.blocks[0])
#                     )
#                 elif isinstance(block, SDCascadeTimestepBlock):
#                     nn.init.constant_(block.mapper.weight, 0)
